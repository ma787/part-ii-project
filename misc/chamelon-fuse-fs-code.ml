...
module Make(Sectors: Mirage_block.S)(Clock : Mirage_clock.PCLOCK) = struct
  ...
  module Traverse = struct
    let rec get_ctz_pointers t l index pointer maxp =
      match l, index with
      | Error _ as e, _ -> Lwt.return e
      | Ok l, 1 -> Lwt.return @@ Ok (pointer :: l, max pointer maxp)
      | Ok l, index ->
        let open Lwt_result.Infix in
        ...
        match pointers with
        | [] -> Lwt.return @@ Ok (pointer::l, maxp)
        | next::_ -> get_ctz_pointers t (Ok (pointer::l)) (index - 1) (Int64.of_int32 next) (max pointer maxp)

    let rec follow_links t visited maxp = function
      | Chamelon.Entry.Data (pointer, length) -> begin
          ...
          get_ctz_pointers t (Ok []) index (Int64.of_int32 pointer) (Int64.of_int32 pointer)
        end
        ...
        | false ->
          ...
          | Ok block ->
            ...
            Lwt_list.fold_left_s (fun so_far link ->
                ...
                | Ok (l, p) ->
                  follow_links t ((a, b)::visited) (max maxp p) link >>= function
                  | Error e ->
                    Log.err (fun f -> f "filesystem seems corrupted; we couldn't get a list of unused blocks: %a" This_Block.pp_error e);
                    Lwt.return @@ Error `Disconnected
                  | Ok (new_links, mp) -> Lwt.return @@ Ok (new_links @ l, mp)
              ) (Ok ([], maxp)) links
            >>= function
            | Ok (list, mp) -> Lwt.return @@ Ok (a :: b :: list, mp)
            | e -> Lwt.return e
  ...
  module Allocate = struct
    ...
    let unused t used_blocks max_pointer =
      ...
      let this_offset = 
        if (max_pointer+1 + alloc_size) >= actual_blocks then 0 else (max_pointer+1) in
      let pool = IntSet.of_list @@ List.init alloc_size
          (fun a -> Int64.of_int @@ a + this_offset)
      in
      this_offset, (this_offset+alloc_size-1), IntSet.(elements @@ diff pool (of_list used_blocks))

    let populate_lookahead ~except t =
      Traverse.follow_links t [] Int64.one (Chamelon.Entry.Metadata root_pair) >|= function
      | Error e ->
        Log.err (fun f -> f "error attempting to find unused blocks: %a" This_Block.pp_error e);
        Error `No_space
      | Ok (used_blocks, max_pointer) ->
        Log.debug (fun f -> f "%d blocks used: %a" (List.length used_blocks) Fmt.(list ~sep:sp uint64) used_blocks);
        Ok (unused t (used_blocks @ except) (max !(t.lookahead).tail (Int64.to_int max_pointer)))
      ...
      if n <= 0 then Lwt.return @@ Ok []
        ...
        let rec aux t acc n =
          ...
          | l ->
            ...
            | new_offset, new_tail, next_l ->
              t.lookahead := ({offset = new_offset; tail = new_tail; blocks = next_l});
  ...
  module File_read : sig
    ...
    let rec read_cstruct cs off =
      if off=(Cstruct.length cs) then cs
      else if Cstruct.get_char cs off = eof then Cstruct.sub cs 0 off
      else read_cstruct cs (off+1)
    
    let get_ctz t key (pointer, file_size) =
      let last_index = Chamelon.File.last_block_index ~file_size
      ...
      let rec read_block l index pointer =
        ...
        | Ok () ->
          ...
          let actual_data = if index=last_index then read_cstruct data_region data_region.off else data_region in
          ...
          match pointers with
          | next::_ -> read_block (actual_data :: l) (index - 1) (Int64.of_int32 next)
          | [] -> Lwt.return @@ Ok (actual_data :: l) in
      Log.debug (fun f -> f "last block has index %d (file size is %d)" last_index file_size);
      read_block [] last_index pointer >>= function
      ...
      if desired_index = index then Lwt.return @@ Ok pointer
        ...
        | Ok () ->
           ...
           * we just have to iterate backward until we get there *)
            ...
            | next :: n_div_2 :: _ ->
              if Int32.(equal n_div_2 max_int) then
                address_of_index t ~desired_index (Int64.of_int32 next, (index - 1))
              else
      ...
      let rec read_raw_blocks ~offset_index l index pointer =
        ...
        | Ok () ->
          ...
          let accumulated_data = 
            if index=last_overall_block_index then (read_cstruct data_region data_region.off) :: l
            else data_region :: l in
  ...
  module File_write : sig
    ...
    val append_in_directory : directory_head -> t -> string -> string ->
      Chamelon.Entry.t -> Chamelon.Entry.t -> (unit, write_error) result Lwt.t
    ...
    let rec write_ctz_block t blocks written index so_far data =
      if Int.compare so_far (String.length data) >= 0 then begin
        ...
        | block_number::blocks ->
          ...
          let can_write = t.block_size - skip_list_length in
          let data_length = min can_write ((String.length data) - so_far) in
          ...
          for n_skip_list = 1 to (skip_list_size - 1) do
            ...
            let point_index = 
              try List.assoc destination_block_index written with Not_found -> Int32.max_int in
            Cstruct.LE.set_uint32 block_cs (n_skip_list * 4) point_index
          done;
          let sub_s, off, len = 
          if can_write > data_length then (String.sub data so_far data_length) ^ (String.make 1 eof), 0, (data_length+1)
          else data, so_far, data_length in
          Cstruct.blit_from_string sub_s off block_cs skip_list_length len;
    ...
    let append_ctz (pointer, file_size) t data =
      let rec read_string cs off =
        if off=(Cstruct.length cs) then Cstruct.to_string cs, off
        else if Cstruct.get_char cs off = eof then Cstruct.to_string cs ~len:off, off
        else read_string cs (off+1) in
      let data_length = String.length data in
      let last_block_index = Chamelon.File.last_block_index ~file_size:(file_size+data_length) ~block_size:t.block_size in
      let prev_last_block_index = Chamelon.File.last_block_index ~file_size ~block_size:t.block_size in
      Allocate.get_blocks t (last_block_index - prev_last_block_index) >>= (function
      | Error _ as e -> Lwt.return e
      | Ok blocks -> 
        let last_block = Cstruct.create t.block_size in
        This_Block.read t.block (Int64.of_int32 pointer) [last_block] >>= function
        | Error _ -> Lwt.return @@ Error (`Not_found Mirage_kv.Key.empty)
        | Ok () ->
          let _pointers, data_region = Chamelon.File.of_block prev_last_block_index last_block in
          let last_block_data, offset = read_string data_region 0 in
          let space = (Cstruct.length data_region) - (String.length last_block_data) in
          Cstruct.blit_from_string data 0 last_block (offset+data_region.off) space;
          let rest_of_data = String.sub data space (data_length - space) in
          let _ = if space > 0 then 
            This_Block.write t.block (Int64.of_int32 pointer) [last_block] >>= (function
            | Error _ as e -> Lwt.return e
            | Ok () -> Lwt.return @@ Ok ()) else Lwt.return @@ Ok () in 
          Log.debug (fun f -> f "got %d blocks from allocate call: %a" (List.length blocks) Fmt.(list ~sep:sp int64) blocks);
          write_ctz_block t blocks [(prev_last_block_index, pointer)] (prev_last_block_index+1) 0 rest_of_data)
    ...
    let rec write_in_ctz dir_block_pair t filename data entries =
      let write_metadata root last_pointer file_size entries =
      ...
      | Ok root ->
        ...
        | None ->
          if List.length entries > 1 then 
            let ctz_entry, new_entries = List.hd entries, List.tl entries in 
            begin
            match Chamelon.File.ctz_of_cstruct (snd ctz_entry) with
            | Some (pointer, size) ->
              let file_size = Int32.to_int size + String.length data in
              append_ctz (pointer, Int32.to_int size) t data >>= (function
              | Error _ as e -> Lwt.return e
              | Ok [] -> Lwt.return @@ Error `No_space
              | Ok ((_last_index, last_pointer)::_) -> write_metadata root last_pointer file_size new_entries)
            | None -> Lwt.return @@ Error (`Not_found (Mirage_kv.Key.v filename))
            end
          else write_ctz t data >>= function
            | Error _ as e -> Lwt.return e
            | Ok [] -> Lwt.return @@ Error `No_space
            | Ok ((_last_index, last_pointer)::_) -> 
              write_metadata root last_pointer (String.length data) entries
    ...
    let append_in_directory block_pair t filename data ctz_entry del_entry =
      write_in_ctz block_pair t filename data [ctz_entry; del_entry]
  end
 ...
 * as opposed to the block size recorded in the filesystem's superblock *)
  ...
  let connect ~program_block_size ~block_size device : (t, error) result Lwt.t =
    ...
    | Ok ((), ()) ->
      ...
      | Error _ as e -> Lwt.return e
      | Ok ((rc0, sb0), (rc1, sb1)) ->
        let lookahead = ref {offset = 0; tail = 0; blocks = []} in
        ...
        | Ok (used_blocks, max_pointer) ->
          ...
          let offset, tail, blocks = unused t used_blocks (Int64.to_int max_pointer)  in
          let lookahead = ref {blocks; offset; tail} in
          Lwt_mutex.unlock t.new_block_mutex;
  ...
  let block_count t = This_Block.block_count t.block
